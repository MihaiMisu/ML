...
def single_prediction(model, img_path):
    """
    Returns array of 1s and 0s which represents the pixel mask for what the
    model detected.
    """
...
def load_model_from_state_dict(model_path):
    ...
    try:
        # define an empty module used only to load a trained one
        model = get_instance_segmentation_model(2)
        model.load_state_dict(torch.load(model_path))
        model.eval()
    except Exception as ex:
        print("Error in loading trained model. Possible not right predictions")
        print(str(ex))
